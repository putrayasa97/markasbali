...
import (
	...
	"io"
	"os"
	"sekolahbeta/learninggrpc/proto"
	"strings"
...
func main() {
	...
	streamClient, err := greetService.StoreStream(context.Background())
	if err != nil {
		logrus.Fatalf("Gagal memulai client streaming, err:%s\n", err.Error())
	}
	...
	for i := 0; i < 5; i++ {
		...
		err = streamClient.Send(&proto.GreetStreamRequest{
			Username: usrName,
		})
		if err != nil {
			logrus.Printf("Gagal mengirim data %s, err:%s", usrName, err.Error())
		}
	}

	err = streamClient.CloseSend()
	if err != nil {
		logrus.Fatalf("Gagal menutup koneksi, err:%s", err.Error())
	}

	err = GreetStream(context.Background(), greetService, usrNames)

	if err != nil {
		logrus.Printf("Terjadi error request streaming, err: %s\n", err.Error())
	}
...
func GreetStream(ctx context.Context, client proto.GreetServiceClient, usrNames []string) error {
	usrName := ""
	...
	if err != nil {
		logrus.Fatalf("Terjadi error saat melakukan stream greetings, error: %s", err.Error())
	}
